...
def main():
    ...
    subparsers = parser.add_subparsers(help="Tools for working with a list")

    parser_add = subparsers.add_parser("add", help="Adds new task into the list")

    parser_add.add_argument("title", type=str, nargs="*")
    parser_add.add_argument("-d", "--date", help="Due date if DD.MM.YYYY format")
    parser_add.add_argument("-e", "--description", type=str, nargs="*")
    parser_add.add_argument("-p", "--priority", type=int, default=0, help="Priority of the task. Value 0 for normal and 1 for important")

    parser_add.set_defaults(func=tasks.add_task)

    parser_list = subparsers.add_parser("list", help="Lists all the tasks")
    parser_list.add_argument("-n", "--name", const="tasks", action="store_const", dest="params")
    parser_list.set_defaults(func=tasks.list_tasks)

    parser_remove = subparsers.add_parser("remove", help="Removes a task from the list")
